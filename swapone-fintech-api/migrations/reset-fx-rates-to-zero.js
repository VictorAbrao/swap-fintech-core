...
async function resetFXRatesToZero() {
  try {
    console.log('🔄 Resetting all FX rates fixed_rate_amount to zero...');
    
    const { data: allRates, error: fetchError } = await supabase
      .from('fx_rates')
      .select('id, from_currency, to_currency, fixed_rate_amount');
    ...
    console.log(`\n✅ Successfully reset ${updatedCount} FX rates fixed_rate_amount to zero`);
    ...
    for (const [pair, count] of Object.entries(currencyPairs)) {
      console.log(`   ${pair}: ${count} rate(s)`);
    }
    
    console.log('\n✅ All FX rates fixed_rate_amount have been reset to zero!');
